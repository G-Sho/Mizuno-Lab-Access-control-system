...
import { Clock, Users, Key, LogIn, LogOut, User, UserPlus } from 'lucide-react';
import { initiateGoogleAuth, parseAuthFromUrl, cleanupAuthUrl, mockSlackAuth, AuthUser } from './auth';
import { checkEnvVars } from './debug';
...
const LabAttendanceSystem = () => {
  ...
  // LocalStorageのキー
  const STORAGE_KEYS = {
    users: 'lab-attendance-users',
    logs: 'lab-attendance-logs',
    currentUser: 'lab-attendance-current-user'
  };
  ...
  useEffect(() => {
    ...
    if (savedLogs) {
      setAttendanceLog(JSON.parse(savedLogs));
    }
    if (savedCurrentUser) {
  ...
  useEffect(() => {
    if (attendanceLog.length > 0) {
      localStorage.setItem(STORAGE_KEYS.logs, JSON.stringify(attendanceLog));
    }
  }, [attendanceLog]);
    ...
    if (!existingUser) {
      ...
      const updatedUsers = users.map(u => 
        u.email === authUser.email 
          ? { ...u, name: authUser.name, avatar: authUser.avatar, provider: authUser.provider }
          : u
      );
      setUsers(updatedUsers);
      existingUser = updatedUsers.find(u => u.email === authUser.email)!;
    }
  ...
  const handleRoomToggle = (roomType: keyof Pick<UserData, 'room2218' | 'gradRoom'>) => {
    ...
    setAttendanceLog(prev => [logEntry, ...prev.slice(0, 49)]); // 最新50件を保持
  };
  ...
  const handleKeyToggle = () => {
    ...
    setAttendanceLog(prev => [logEntry, ...prev.slice(0, 49)]);
  };
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-600">ログイン方法を選択してください</p>
          </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-800">研究室入退室管理</h1>
...
export default LabAttendanceSystem;